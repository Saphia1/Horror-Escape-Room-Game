...
class Grid(PygameUtil):
    ...
    def gencells(self):#method to generate the cells within our list
        for i in range(0,self.__height):#dont want to start at 0
    ...
    def buildgrid(self):#method to build the grid, will draw every row and cell in array
        for row in self.__grid:#(row is r it is doing the collumns)
            for cell in row:
                ...
                walls=cell.getwalls()
                if walls["top"]:
                    self.drawline((255,255,255),(x,y),(x+w,y), self._screen)#end point is gained by x+w,y (add the picture from notes in annotations in whiteboard)
                if walls["bottom"]:
                    self.drawline((255,255,255),(x,y+h),(x+w,y+h), self._screen)
                if walls["left"]:
                    self.drawline((255,255,255),(x,y),(x,y+h), self._screen)
                if walls["right"]:
                    self.drawline((255,255,255),(x+w,y),(x+w,y+h), self._screen)
        #grid.buildce
    ...
    def depthfirst(self):#want to start at first cell, pick random neighbour, from random neighbour, then choose random neighbour and so on. If stuck at nay point, backtrack till you can choose another neighbour.
        ...
        while len(visited)<(self.__width*self.__height):#while we havent visisted all the cells
            ...
            if cell not in visited:
                visited.append(cell)
            neighbourcheck=self.findneighbours(cell,visited)
            if neighbourcheck:
    ...
    def findneighbours(self,cell,visited):
        ...
        for i in possibleneighbours:#in dpeth explanation on whitevoard(rearranged an equation from gencells (the for loops to make the grid))
            ...
            i=(neighbour_yvalue//self.__start_pos[1])-1#error, put / instead of // but needed // because integer division
            j=(neighbour_xvalue//self.__start_pos[0])-1
            if 0<=i<=(self.__height-1) and 0<=j<=(self.__width-1) and self.__grid[i][j] not in visited:#if cell exists and also hasnt been visited
                cell.addneighbour(self.__grid[i][j])
        i = (y // self.__start_pos[1]) - 1  # error, put / instead of // but needed // cuz inyteger division
        j = (x // self.__start_pos[0]) - 1
        self.__grid[i][j] = cell
